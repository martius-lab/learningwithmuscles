...
def get_nvidia_lib_dir():
    ...
    if len(paths) > 1:
        print(
            "Choosing the latest nvidia driver: %s, among %s" % (paths[-1], str(paths))
        )
...
"""
    ...
    if sys.platform == "darwin":
        ...
        if (
            os.getenv("MUJOCO_PY_FORCE_CPU") is None
            and get_nvidia_lib_dir() is not None
        ):
        ...
        if var not in os.environ or lib_path not in os.environ[var].split(";"):
            raise Exception(
                "Please add mujoco library to your PATH:\n"
...
def _ensure_set_env_var(var_name, lib_path):
    ...
    if lib_path not in paths:
        raise Exception(
            ...
            "export %s=$%s:%s"
            % (var_name, os.environ.get(var_name, ""), var_name, var_name, lib_path)
        )
...
def manually_link_libraries(mujoco_path, raw_cext_dll_path):
    ...
    if exists(final_cext_dll_path) and getmtime(final_cext_dll_path) >= getmtime(
        raw_cext_dll_path
    ):
...
class MujocoExtensionBuilder:
    ...
    def _build_impl(self):
        dist = Distribution({"script_name": None, "script_args": ["build_ext"]})
        ...
        build.build_base = join(
            self.CYMJ_DIR_PATH, "generated", "_pyxbld_%s" % (self.version)
        )
    ...
    def get_so_file_path(self):
        ...
        python_version = str(sys.version_info.major) + str(sys.version_info.minor)
        return join(
            dir_path, "generated", "cymj_{}_{}.so".format(self.version, python_version)
        )
